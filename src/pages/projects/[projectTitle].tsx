import React, { useEffect, useState } from "react";
import Image from "next/image";
import Link from "next/link";
import { RiRadioButtonFill } from "react-icons/ri";
import { project } from "../../data/projects";
import Router, { useRouter } from "next/router";
import { Project } from "src/types/projects";

const Project = () => {
  const router = useRouter().query.projectTitle as String;
  const [projectById, setProjectById] = useState<Project>();

  useEffect(() => {
    const filterProject = project.find((project) => project.title === router);
    if (filterProject && router !== filterProject.title) {
      Router.push("/#projects");
    }
    setProjectById(filterProject);
  }, [router]);

  return (
    <div className="w-full">
      <div className="w-screen h-[50vh] relative">
        <div className="absolute top-0 left-0 w-full h-[50vh] bg-black/70 z-10" />
        <Image
          className="absolute z-1 object-cover"
          fill
          src="https://res.cloudinary.com/dvtx1e6ag/image/upload/v1689172217/Screenshot_2023-07-12_102937_xqovg1.png"
          alt="/"
        />
        <div className="absolute top-[70%] max-w-[1240px] w-full left-[50%] right-[50%] translate-x-[-50%] translate-y-[-50%] text-white z-10 p-2">
          <h2 className="py-2">{projectById?.title}</h2>
          <h3>{projectById?.subtitle}</h3>
        </div>
      </div>

      <div className="max-w-[1240px] mx-auto p-2 grid md:grid-cols-5 gap-8 py-8">
        <div className="col-span-4">
          <p>Project</p>
          <h2 className="py-6">Overview</h2>
          <p>{projectById?.overview}</p>

          <Link href="https://pastry-wheat.vercel.app/" rel="noreferrer">
            <button className="px-8 py-2 mt-4">Demo</button>
          </Link>
        </div>
        <div className="col-span-4 md:col-span-1 shadow-xl shadow-gray-400 rounded-xl py-4">
          <div className="p-2">
            <p className="text-center font-bold pb-2">Technologies</p>
            <div className="grid grid-cols-3 md:grid-cols-1">
              {projectById?.technologies.map((tech, index) => (
                <>
                  <p
                    key={index}
                    className="text-gray-600 py-2 flex items-center"
                  >
                    <RiRadioButtonFill className="pr-1" /> {tech}
                  </p>
                </>
              ))}
            </div>
          </div>
        </div>
        <Link href="/#projects" scroll={false}>
          <p className="underline cursor-pointer">Back</p>
        </Link>
      </div>
    </div>
  );
};

export default Project;
